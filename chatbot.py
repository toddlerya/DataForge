...
from agent.input_analyze_agent.intent_agent import intent_graph
from agent.input_analyze_agent.state import UserIntentSchema
...
async def input_intent_analyze(user_input: str, thread: dict) -> UserIntentSchema:
    ...
    event = await cl.make_async(intent_graph.invoke)({"user_input": user_input}, thread, stream_mode="values")
...
async def main(message: cl.Message):
    ...
    if message.content.strip() == "正确":
        ...
        intent_graph.update_state(thread, {"confirmed": True}, as_node="confirmed")
        ...
        try:
            ...
            await cl.Message(content=user_intent.model_dump_json(indent=2), language="python").send()
            # 发送响应给用户
