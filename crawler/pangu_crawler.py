...
class PanGuCrawler:
    ...
    def run(self, overwrite: bool = False):
        ...
        for entity_info in self.entity_elements:
            ...
            # 采集表的样例数据
            example_data = self.crawl_sample(table_en_name=each_table_metadata_model.table_en_name,
                                             entity_id=entity_id,
                                             limit=100)
            logger.debug(f"entity_id={entity_id} table_en_name={each_table_metadata_model.table_en_name} "
            ...
            if save_status is False:
                logger.error(f"盘古元数据信息入库异常: {each_table_metadata_record} ERROR: save_message")
