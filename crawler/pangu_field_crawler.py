    ...
    def fetch_pangu_field_recommend_info(self) -> bool:
        ...
        for index, each_field_data in enumerate(self.all_field_stat_data, start=1):
            ...
            status, message, data = pangu_recommend_field_info(
                db_handler=self.metadata_db, field_en_name=field_en_name
            )
            if status is False:
                logger.error(
            ...
            save_filed_status, save_field_message = save_recommend_pangu_field_info(
                db_handler=self.inner_db, recommend_pangu_field_data=data.model_dump()
            )
            if save_filed_status is False:
                logger.error(
                    f"存储盘古字段推荐异常: field_en_name={field_en_name} ERROR: {save_field_message}"
                )
                return False
    ...
    def fetch_pangu_dict_info(self) -> bool:
        ...
        for index, dictkey_with_nlevel in enumerate(
            ...
            status, message, data = pangu_dict_key_values(
                db_handler=self.metadata_db, dictkey_with_nlevel=dictkey_with_nlevel
            )
            if status is False:
                logger.error(
                    f"获取盘古字典异常: dictkey_with_nlevel={dictkey_with_nlevel} ERROR: {message}"
                )
                return False
            for each_data in data:
                save_status, save_message = save_pangu_dict_info(
                    db_handler=self.inner_db, pangu_dict_key_data=each_data
                )
...
if __name__ == "__main__":
    db_handler = Database()
    pfc = PanGuFieldCrawler(inner_db=db_handler)
    pfc.run()
