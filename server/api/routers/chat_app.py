...
router = APIRouter(prefix="/chat", tags=["数据生成", "chat"], responses={404: {"description": "ChatApp Not Found"}})
...
async def data_gen_app(path: str):
    return await run_chainlit(str(PROJECT_PATH.joinpath("app_data_gen", "chatbot.py").absolute()))
...
async def table_gen_app(path: str):
    return await run_chainlit(str(PROJECT_PATH.joinpath("app_table_gen", "app.py").absolute()))
