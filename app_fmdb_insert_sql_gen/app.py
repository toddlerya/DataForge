...
上传你的表元数据配置JSON和DG生成的数据TXT文件"""
    ...
    try:
        with open(
            ...
            await cl.Message(
                content=f"表名称为: {table_meta_data.table_en_name}, 有{len(table_meta_data.raw_fields_info)}列字段"
            ).send()
        ...
        try:
            with open(
                ...
                if len(set(lines_col_count)) == 1 and list(set(lines_col_count))[
                    ...
                    await cl.Message(
                        content=f"上传的数据文件: {dg_table_data_txt_file.name} 解析校验异常, 列数量不一致, 期望{cl.user_session.get('col_number')}列, 实际{list(set(lines_col_count))}列"
                    ).send()
                    continue
        except Exception as err:
            await cl.Message(
    ...
    # 开始拼接SQL
    insert_sql = f"INSERT INTO {table_meta_data.table_en_name} \n"
    if partition_sql:
        insert_sql += partition_sql
    insert_sql += "VALUES \n"
