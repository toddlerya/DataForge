...
class RecommendPanGuFieldSchema(BaseModel, extra="forbid", str_strip_whitespace=True):
    ...
    description: str = Field("", description="出现次数最多的字段描述，对应COMMENT")
    ...
    field_length: str = Field("", description="出现次数最多的字段长度")
    ...
    structure_type: int = Field(0, description="出现次数最多的字段结构化类型，0(默认)：结构化；1：非结构化")
    ...
    is_multi_value: int = Field(0, description="出现次数最多的是否是多值列 1：是；0：否")
