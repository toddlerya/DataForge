...
def table_metadata_query(table_en_name: str, db_handler: Database) -> Tuple[bool, str, TableMetaDataInfo | None]:
    """
    查询数据
    Args:
        table_en_name:
    ...
    try:
        result = db_handler.session.query(TableMetaDataInfo).filter(
            TableMetaDataInfo.table_en_name == table_en_name).one_or_none()
    except Exception as err:
