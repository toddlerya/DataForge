...
from config import HEADERS, PROXIES, ENV_APOLLO_IP, ENV_APOLLO_WEB_PORT
...
class Apollo:
    def __init__(self,
        ...
        # 阿波罗配置中心
        self.APOLLO_CREATE_APP_URL = f"https://{self.ip}:{self.port}/openapi/v1/apps/create-app"
        self.APOLLO_CREATE_APP_PAYLOAD = {
            "appId": "Polaris",
            "name": "北极星"
        }
        self.APOLLO_CREATE_AUTHORIZATION_HEADER = {"Authorization": "f1ae5a2e2f6967719f0b7b360a62bd36b146e42b"}
        self.APOLLO_FETCH_URL = f"https://{self.ip}:{self.port}" \
    ...
    def auth(self) -> tuple[bool, str]:
        ...
        try:
            resp = requests.post(url=self.APOLLO_CREATE_APP_URL,
                                 headers=headers,
                                 proxies=PROXIES,
                                 verify=False,
                                 timeout=60,
            ...
            if resp_data.get("status", "") != "OLD":
                message = (f"不应出现此情况: [POST] URL: {self.APOLLO_CREATE_APP_URL} "
    ...
    def fetch_all_config(self) -> tuple[bool, str, list[dict]]:
        """
        获取全部配置
        Returns:
        ...
        try:
            resp = requests.get(url=self.APOLLO_FETCH_URL, headers=headers, proxies=PROXIES, verify=False, timeout=60)
        except Exception as err:
            message = f"调用{self.APOLLO_FETCH_URL}接口异常: {err}"
            return False, message, [{}]
        else:
            if resp.status_code != 200:
                message = (f"获取Apollo的全部配置失败, [GET] URL: {self.APOLLO_FETCH_URL} "
        ...
        if resp_data.get("appId", "") != self.app_id:
            message = (
                f"[appId!={self.app_id}]不应出现此情况: [GET] URL: {self.APOLLO_FETCH_URL} HEADERS: {headers} "
                f"RESPONSE: appId={resp_data.get('appId')}, 与请求的appId={self.app_id}不同")
            return False, message, [{}]
    ...
    def query_value_by_key(self, key: str) -> tuple[bool, str]:
        ...
        try:
            resp = requests.get(url=url, headers=headers, proxies=PROXIES, verify=False, timeout=60)
        except Exception as err:
            logger.error(f"调用{url}接口异常: {err}")
            return False, ""
        else:
            if resp.status_code != 200:
                logger.error(f"获取Apollo的{key}的配置值失败, HTTP_STATUS_CODE: {resp.status_code}")
                return False, ""
            resp_data = resp.json()
            if resp_data.get("key", "") != key:
                logger.warning(f"不应出现此情况: [GET] URL: {url} HEADERS: {headers} RESPONSE: {resp_data}")
            value = resp_data.get("value", "未获取到value")
            return True, value

    def fetch_and_format_all_config(self) -> tuple[bool, str, [dict]]:
        """
        ...
        if auth_status is False:
            return auth_status, f"apollo获取token失败: {auth_message}", {"config_list": result_array}
        ...
        if _status:
            ...
            _message = f"获取阿波罗配置中心数据，格式化为PolarisInspectionConfig的结果结构失败: {_message}"
...
if __name__ == '__main__':
    ...
    test_env = "172.16.44.125"
    dev_env = "172.16.110.144"
    dev3_env = "172.16.112.99"
    apollo = Apollo(ip=dev3_env)
