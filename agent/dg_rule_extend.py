...
def force_update_dg_rule(pydantic_data_genius_rule: PydanticDataGeniusRule):
    # 依次进行检查若符合条件则应用更新规则
    pydantic_data_genius_rule = force_update_dg_timestamp_rule(
        pydantic_data_genius_rule
    )
    pydantic_data_genius_rule = force_update_dg_data_color_id_rule(
        pydantic_data_genius_rule
    )
...
def force_update_dg_timestamp_rule(pydantic_data_genius_rule: PydanticDataGeniusRule):
    """
    强制设置DG的时间戳规则
    1. LLM推荐的score为0时, 针对字段名称为明显的UNIX时间戳字段的, 程序固定回填时间戳,
        时间戳默认使用`当前时间绝对秒`
    ...
    # 情况1：部分时间字段推荐不准, 兜底逻辑
    if (
        pydantic_data_genius_rule.preview.startswith("score: 0")
        and "_TIME" in pydantic_data_genius_rule.ename.upper()
    ):
        # "preview": "score: 0, reason: 无法找到合适的类别"  && "ename": "DELETE_TIME"
        pydantic_data_genius_rule.category = "当前时间绝对秒"
        pydantic_data_genius_rule.preview += (
            "【时间字段-由程序强制设置规则为当前时间绝对秒】"
        )
...
def force_update_dg_datetime_rule(pydantic_data_genius_rule: PydanticDataGeniusRule):
    ...
    if (
        pydantic_data_genius_rule.preview.startswith("score: 0")
        and "_DATE" in pydantic_data_genius_rule.ename.upper()
    ):
...
def force_update_dg_data_color_id_rule(
    ...
    if "DATA_COLOR_ID" in pydantic_data_genius_rule.ename.upper():
        ...
        pydantic_data_genius_rule.args = {
            "head": "dg_",
            "chars_in": "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ",  # noqa: E501
            "tail": "",
            "max_": "32",
            "min_": "32",
        }
...
if __name__ == "__main__":
    rule1 = PydanticDataGeniusRule(
        **{
            ...
            "cname": "提取原因",
            "preview": (
                "score: 0, reason: 字段示例数据为JSON格式的数组, 包含多个对象, "
                "每个对象有'mrk'、'dir'、'status'等键, 这与预定义的任何类别都不匹配。"
                "经过仔细检查, 没有发现符合的类别, 因此置信度设为0。"
            ),
    ...
    rule2 = PydanticDataGeniusRule(
        **{
            ...
            "cname": "首次关联时间",
            "preview": (
                "score: 90, reason: 字段示例数据'1745333599'是一个时间戳, "
                "表示自1970年1月1日以来的秒数, 与<configs>中'时间绝对秒'条目匹配。"
            ),
            "value": "1745333599",
            "args": {},
        }
    ...
    rule3 = PydanticDataGeniusRule(
        **{
            ...
            "cname": "属性类型二的值",
            "preview": (
                "score: 90, reason: 字段示例数据'18871733578'是11位数字, "
                "符合中国大陆手机号码的格式, 与<configs>中'手机号'条目匹配。"
            ),
            "value": "18871733578",
            "args": {},
        }
    ...
    rule4 = PydanticDataGeniusRule(
        **{
            ...
            "cname": "来源",
            "preview": (
                "score: 0, reason: 无法找到合适的类别, "
                "因为'137C005'包含字母和数字, 但没有匹配的预定义类别。"
            ),
            "value": "137C005",
            "args": {},
        }
