...
def recommend_dg_rule_by_llm(structured_llm,
    ...
    if retry_count >= max_retries:
        ...
        return retry_count, pydantic_data_genius_rule
...
def dg_rule_processor(state: Union[SQLModeDataGenState, DataGenState]
    ...
    table_en_name = user_intent.table_en_names[0] or table_metadata_info.table_en_name
    row_count = user_intent.data_count or user_intent.table_data_count.get(table_en_name, 1000)
    table_metadata: TableMetadataSchema = table_metadata_array[0] if table_metadata_array else table_metadata_info
    ...
    # 如果是 SQLModeDataGenState，需要从 table_info_data 中提取 table_metadata_array
    if isinstance(state, dict) and "table_info_data" in state:
        table_info_data = state.get("table_info_data", {})
        table_metadata_array = table_info_data.get("table_metadata_array", [])
    ...
    while True:
        ...
        retry_count, pydantic_data_genius_rule = recommend_dg_rule_by_llm(structured_llm=structured_llm,
